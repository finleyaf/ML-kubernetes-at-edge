...
This gap (the absence of a lightweight system that combines both anomaly detection and short-term prediction to inform scheduling) motivates the present work. This project aims to integrate a proven lightweight anomaly detection method with a resource prediction model within a custom Kubernetes scheduling component. By unifying detection and prediction in a scheduler-aware pipeline, the system seeks to avoid placing workloads on anomalous or soon-to-be-overloaded nodes, thereby improving scheduling latency, pod start-up time, and resource utilisation compared to the default Kubernetes scheduler. Methodologically, the project separates offline development from final live evaluation: replication and offline replay are used to justify detector choice, predictor adequacy, and the locked hybrid policy family before the final online run is observed. The final scheduler comparison is then made through an authoritative matched-arm experiment on a live K3s cluster, with both arms restricted to a homogeneous three-worker subset so that the default scheduler remains methodologically interpretable while dynamic load is still present \cite{liu2024dynamic}.
...
\begin{enumerate}
    ...
    \item \textbf{Extend with lightweight prediction and custom scheduling.} Design and implement a lightweight machine learning model to predict short-term node resource utilisation. Integrate both the anomaly detector and the predictor into a custom scheduling component that scores nodes using predicted load, anomaly risk, and live workload-capacity context.
    
    \item \textbf{Evaluate and compare.} Experimentally evaluate the integrated system against the default Kubernetes scheduler in a live K3s edge cluster, using a homogeneous worker subset for the final authoritative comparison. Compare results on key performance metrics: scheduling latency, pod start-up time, placement safety, and resource contention.
\end{enumerate}
...
For edge deployments, the full Kubernetes distribution carries considerable operational overhead in binary size, memory footprint, and dependency complexity. K3s is a lightweight, CNCF-certified Kubernetes distribution that reduces this footprint by packaging dependencies into a single binary, replacing etcd with a lighter datastore, and automating TLS management \cite{kim2025exploring}. K3s retains full API compatibility with standard Kubernetes, making it suitable for resource-constrained edge nodes. The replication studies that informed detector choice are provided in Appendix~\ref{chapter2}, while the final experimental methodology is defined in Chapter~\ref{chapter3}.
